...
struct PromoCodeModel: Identifiable, Equatable{
    ...
    init(dict: NSDictionary) {
        self.promoCodeId = dict.value(forKey: "promo_code_id") as? Int ?? 0
        self.title = dict.value(forKey: "code") as? String ?? ""
        self.code = dict.value(forKey: "code") as? String ?? ""
        self.description = dict.value(forKey: "title") as? String ?? ""
        self.type = dict.value(forKey: "type") as? Int ?? 0
        self.minOrderAmount = dict.value(forKey: "min_order_amount") as? Double ?? 0.0
        self.maxDiscountAmount = dict.value(forKey: "max_discount_amount") as? Double ?? 0.0
