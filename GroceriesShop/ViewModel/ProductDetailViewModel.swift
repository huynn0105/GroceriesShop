...
class ProductDetailViewModel: ObservableObject {
    ...
    func serviceCallDetail(){
        ServiceCall.post(parameter: ["prod_id":self.pObj.productId], path: Globs.SV_PRODUCT_DETAIL, isToken: true) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: KKey.status) as? String ?? "" == "1" {
                    ...
                    if let payloadObj = response.value(forKey: KKey.payload) as? NSDictionary {
                        ...
                        self.imageArr = (payloadObj.value(forKey: "image_list") as? NSArray ?? []).map({ obj in
    ...
    func serviceCallAddRemoveFavorite(){
        ServiceCall.post(parameter: ["prod_id":self.pObj.productId], path: Globs.SV_ADD_REMOVE_FAVORITE, isToken: true) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: KKey.status) as? String ?? "" == "1" {
